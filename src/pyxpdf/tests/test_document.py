...
class DocumentTextCase(InitGlobalTextCase, PropertyTextCase):
    ...
    dmca_prop = {
        ...
        "info()": {
            "Creator": "C:My DocumentsSUMMARY6.WPD",
            ...
            "Keywords": (
                "digital millennium copyright act circumvention technological "
                "protection management information online service provider "
                "liability limitation computer maintenance competitiion repair "
                "ephemeral recording webcasting distance education study vessel "
                "hull"
            ),
    ...
    def _test_doc_properties(self, doc):
        for prop, value in self.dmca_prop.items():
            with self.subTest("testing prop {0}".format(prop)):
                if "()" in prop:
                    self.assertEqual(
                        getattr(doc, prop.rstrip("()"))(),
                        value,
                        "assertEqual {0}".format(prop),
                    )
                else:
                    self.assertEqual(
                        getattr(doc, prop), value, "assertEqual {0}".format(prop)
                    )
